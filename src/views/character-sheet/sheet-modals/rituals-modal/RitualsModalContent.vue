<script setup lang="ts">
import { computed, ref } from 'vue'
import { Ritual } from '../../../../types'
import rituals from '../../../../data/rituals'
import TabNav from '../../../../components/TabNav.vue'
import RitualCard from '../../../../components/RitualCard.vue'
import SearchInput from '../../../../components/SearchInput.vue'
import { compare } from '../../../../utils/functions'

const emit = defineEmits(['handleAddRitual', 'handleCloseModal'])

const tabOptions = [
  {
    label: 'Todos',
    value: 0
  },
  {
    label: 'Conhecimento',
    value: 1,
  },
  {
    label: 'Energia',
    value: 2,
  },
  {
    label: 'Morte',
    value: 3,
  },
  {
    label: 'Sangue',
    value: 4,
  },
  {
    label: 'Medo',
    value: 5,
  },
  {
    label: 'Varia',
    value: 6,
  }
]

const secondaryTabOptions = [
  {
    label: 'Todos',
    circle: '-',
    value: 0
  },
  {
    label: '1º Círculo',
    circle: '1',
    value: 1
  },
  {
    label: '2º Círculo',
    circle: '2',
    value: 2
  },
  {
    label: '3º Círculo',
    circle: '3',
    value: 3
  },
  {
    label: '4º Círculo',
    circle: '4',
    value: 4
  }
]

const currentTab = ref(0)
const currentSecondaryTab = ref(0)
const searchText = ref('')

const handleAddRitual = (ritual: Ritual) => emit('handleAddRitual', ritual)

const currentRituals = computed<Ritual[]>(() => {
  if(currentTab.value === 0) {
    if(currentSecondaryTab.value === 0) return rituals.filter((ele) => compare(ele.name, searchText.value)).sort((a, b) => a.name.localeCompare(b.name))
    else return rituals.filter((ritual) => ritual.circle === secondaryTabOptions[currentSecondaryTab.value].circle)
                        .filter((ele) => compare(ele.name, searchText.value)).sort((a, b) => a.name.localeCompare(b.name))
  }

  if(currentSecondaryTab.value === 0) {
    if(currentTab.value === 0) return rituals.filter((ele) => compare(ele.name, searchText.value)).sort((a, b) => a.name.localeCompare(b.name))
    else return rituals.filter((ritual) => ritual.element === tabOptions[currentTab.value].label)
                        .filter((ele) => compare(ele.name, searchText.value)).sort((a, b) => a.name.localeCompare(b.name))
  }

  return rituals.filter((ritual) => ritual.element === tabOptions[currentTab.value].label).filter((ele) => compare(ele.name, searchText.value))
          .filter((ritual) => ritual.circle === secondaryTabOptions[currentSecondaryTab.value].circle).filter((ele) => compare(ele.name, searchText.value))
})
</script>

<template>
  <div class="rituals-modal-content">
    <div class="class-rituals-container">
      <TabNav
        :current-tab="currentTab"
        :tab-options="tabOptions"
        @handle-navigation="(value: number) => currentTab = value"
      />
      <div class="secondary-tab-container">
        <TabNav
          :current-tab="currentSecondaryTab"
          :tab-options="secondaryTabOptions"
          secondary
          @handle-navigation="(value: number) => currentSecondaryTab = value"
        />
      </div>
      <div class="search-container">
        <SearchInput 
          :value="searchText"
          dark
          @update="value => searchText = value"
        />
      </div>
      <div
        v-if="currentRituals.length > 0"
        class="class-rituals-content"
      >
        <div
          v-for="ritual in currentRituals"
          :key="ritual.name"
          class="class-rituals-card"
        >
          <RitualCard
            :ritual="ritual"
            @handle-add="handleAddRitual"
          />
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.rituals-modal-content {
  margin-top: 1rem;
}
.secondary-tab-container {
  margin-top: 1rem;
}
.class-rituals-content {
  margin-top: 1rem;
  border-radius: 4px;
  padding: .5rem;
  background-color: var(--color-smoky-black);
}
.class-rituals-card:not(:last-of-type) {
  margin-bottom: .5rem;
}
.search-container {
  margin-top: 1rem;
}
</style>